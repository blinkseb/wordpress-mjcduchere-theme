...
gulp.task('styles-rubysass', function() {
  ...
  .pipe(gulp.dest(config.build.dest + 'css')); // Render the final CSS file(s) into the `build` folder
});
...
gulp.task('styledotcss', function() {
  return gulp.src(config.build.root + 'style.css')
  .pipe(gulp.dest(config.build.dest));
});


// Easily configure the Sass compiler from `/gulpconfig.js`
gulp.task('styles', ['styledotcss', 'styles-'+config.compiler]);
